...
if (len(sys.argv) != 3):
	...
	print 'Usage: python CellProfiler_plot_bins.py /path/to/Green.csv /path/to/Red.csv'
	sys.exit()

GFP_data = NP.genfromtxt(sys.argv[1], delimiter=',', skip_header=1, skip_footer=0, usecols=(4, 9, 10), names=['f_num', 'AreaShape_Center_X', 'AreaShape_Center_Y'])

mCherry_data = NP.genfromtxt(sys.argv[2], delimiter=',', skip_header=1, skip_footer=0, usecols=(4, 9, 10), names=['f_num', 'AreaShape_Center_X', 'AreaShape_Center_Y'])
...
for i in range(0, len(GFP_data)):
	...
	frame = int(GFP_data['f_num'][i])
	green_X = int(GFP_data['AreaShape_Center_X'][i])
...
for i in range(0, len(mCherry_data)):
	...
	frame = int(mCherry_data['f_num'][i])
	red_X = int(mCherry_data['AreaShape_Center_X'][i])
...
PLT.savefig('CellProfiler_Bins_1-4.png', dpi=120)
...
axarr[1, 1].set_xlim(xr)
axarr[1, 1].set_ylim(yr)
PLT.savefig('CellProfiler_Bins_5-8.png', dpi=120)
