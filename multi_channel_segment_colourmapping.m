...
function [] = multi_channel_segment_colourmapping()
    ...
    if (length(phaseImgs) ~= length(mCherryImgs)) || (length(phaseImgs) ~= length(GFPImgs))
        ...
        disp(length(phaseImgs));
        disp(length(mCherryImgs));
        disp(length(GFPImgs));
        return;
    end
    ...
    for i = 1 : length(phaseImgs)
        ...
        [I_gray_bw, I_gray_artifacts] = binarize_phase(Igray, 10, 50, display);
		...
		dlmwrite(GFPCSVfile, res_green, '-append', 'precision', '%.3f', 'delimiter', ',');
		dlmwrite(mCherryCSVfile, res_red, '-append', 'precision', '%.3f', 'delimiter', ',');
...
function [image_process] = colour_mapping(image)
    prc = prctile(image(:),99);
    prc = double(prc);
    map = gray(prc);
    image_process = ind2gray(image,map);
    %%image_double = double(image_process);
...
function [I_red_enhanced, I_green_enhanced] = enhance_contrast(Igray, I_gray_artifacts, Igreen, Ired, Igout, Irout, disp)
    ...
    Igreen = colour_mapping(Igreen);
    Ired = colour_mapping(Ired);
    Igray = colour_mapping(Igray);
    ...
    I_green_enhanced(I_green_enhanced < prctile(uniqLevels, 20)) = 0;
    I_green_enhanced = imfill(adapthisteq(medfilt2(I_green_enhanced)));
    
    uniqLevels = unique(I_red_enhanced(:));
    I_red_enhanced(I_red_enhanced < prctile(uniqLevels, 20)) = 0;
...
%% Binarize phase
function [I_gray_bw_res, I_gray_artifacts] = binarize_phase(Igray, shift, minobjsize, disp)
    ...
    I_gray_bw_res = I_gray_bw & ~I_gray_artifacts;
    ...
    if (usejava('desktop') == 1 && disp == 1)
        ...
        subplot(2,2,4), imshow(I_gray_bw_res), title('Updated phase binary mask')
    end
...
function [s_green, s_red] = watershed_segment(I_green_enhanced, I_red_enhanced, Igout, Irout, Ipout, disp)
    ...
    if (usejava('desktop') ~= 1 || disp == 0)
        %set(hax, 'Visible', 'off');
        %set(hax, 'position', [0 0 1 1], 'units', 'normalized');
        %imshow(out_image);
        %saveas(hax, strcat('Watershed-Outline', filesep, Ipout, '.png'));
        imwrite(out_image, strcat('Watershed-Outline', filesep, Ipout, '.png'));
    else
        imshow(out_image);
        print(strcat('Watershed-Outline', filesep, Ipout, '.png'), '-dpng');
    end
    
    green_outline = I_green_enhanced;
    [I, J] = find(green_borders > 0);
    ind = sub2ind(size(I_green_enhanced), I, J);
    green_outline(ind) = 255;
    Seg_green = cat(3, I_green_enhanced, green_outline, I_green_enhanced);
    
    % Create figure showing green boundaries
    if (usejava('desktop') ~= 1 || disp == 0)
        imwrite(Seg_green, strcat('Watershed-Green-Outline', filesep, Igout, '.png'));
    end
    
    red_outline = I_red_enhanced;
    [I, J] = find(red_borders > 0);
    ind = sub2ind(size(I_red_enhanced), I, J);
    red_outline(ind) = 255;
    Seg_red = cat(3, red_outline, I_red_enhanced, I_red_enhanced);
    
    % Create figure showing red boundaries
    if (usejava('desktop') ~= 1 || disp == 0)
        imwrite(Seg_red, strcat('Watershed-Red-Outline', filesep, Irout, '.png'));
    end
